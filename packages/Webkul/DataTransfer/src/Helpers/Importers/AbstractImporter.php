    ...
    public function validateData(): void
    {
        ...
        foreach ($this->getSource()->getColumnNames() as $columnNumber => $columnName) {
            if (empty($columnName)) {
                ...
                !in_array($columnName, $this->getValidColumnNames()) &&
                !collect($this->getValidColumnNames())->contains(function ($validName) use ($columnName) {
                    return $validName[0] === '/' && preg_match($validName, $columnName);
                })
            ) {
