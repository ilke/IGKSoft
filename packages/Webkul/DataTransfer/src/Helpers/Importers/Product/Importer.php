...
class Importer extends AbstractImporter
{
    ...
    protected function initAttributes(): void
    {
        ...
        // Dinamik olarak imageX_url başlıklarını geçerli kabul et
        $this->validColumnNames[] = '/^image\d+_url$/';
    }
    ...
    public function prepareImages(array $rowData, array &$imagesData): void
    {
        ...
        // Collect from 'images' column if present
        if (!empty($rowData['images'])) {
            $imageNames = array_map('trim', explode(',', $rowData['images']));
        }

        // Collect from image1_url, image2_url, ... columns
        foreach ($rowData as $key => $value) {
            if (preg_match('/^image\d+_url$/', $key) && !empty($value)) {
                $imageNames[] = trim($value);
            }
        }
        ...
        foreach ($imageNames as $key => $image) {
            if (filter_var($image, FILTER_VALIDATE_URL)) {
                ...
                $filename = uniqid('imported_') . '.' . $extension;
                $importDir = storage_path('app/import/' . $this->import->images_directory_path);
                if (!is_dir($importDir)) {
                    mkdir($importDir, 0777, true);
                }
                $importPath = $importDir . '/' . $filename;
                file_put_contents($importPath, $imageContents);
                $imagesData[$rowData['sku']][] = [
                    'name' => $filename,
                    'path' => $importPath,
                ];
